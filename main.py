...
intents = Intents(
  guilds=True, messages=True, reactions=True, message_content=True, voice_states=True
)
bot = ShardedBot(intents=intents, activity=discord.Game(name="/ translations"))
...
async def on_guild_join(guild):
  ...
  await client.post(
    url=WEBHOOK_URL, data=json.dumps(payload).encode("utf-8"), headers=headers
  )
...
async def on_guild_remove(guild):
  ...
  await bot.cursor.execute(
    f"DELETE FROM default_guild_language WHERE guild_id = {guild.id}"
  )
  await bot.db.commit()
