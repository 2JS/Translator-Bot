...
class LinkChannels(Cog):
  ...
  async def linkchannels_add(
    self, ctx: ApplicationContext, channel_1, channel_2, language_1, language_2
  ):
    ...
    if channel_1 == channel_2:
      ...
      await self.bot.cursor.execute(
        f"SELECT * FROM linkchannels WHERE channel_id_1 = {channel_1.id} OR channel_id_2 = {channel_1.id} OR channel_id_1 = {channel_2.id} OR channel_id_2 = {channel_2.id}"
      )
  ...
  @linkchannels.command(
    name="list", description="List all the linked channels in the server."
  )
  async def linkchannels_list(self, ctx: ApplicationContext):
    await ctx.defer(ephemeral=True)

    await self.bot.cursor.execute(
      f"SELECT * FROM linkchannels WHERE guild_id = {ctx.guild.id}"
    )
    LinkChannelsList = await self.bot.cursor.fetchall()
    ...
    if LinkChannelsList == []:
      ...
      EmbedMessage = Embed(title="Link Channels list", color=0x5865F2)
      ...
      if len(LinkChannelsList) <= 25:
        ...
        for x in range(24, len(LinkChannelsList)):
          ...
          EmbedMessage.description += (
            f"<#{channel_1}> <-> <#{channel_2}> : {language_1} <-> {language_2}\n"
          )
