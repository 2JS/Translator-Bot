...
class PersonalCommand(Cog):
  ...
  async def personal(self, ctx: ApplicationContext, language):
    ...
    if language != "none":
      ...
      if result:
        await cursor.execute(
          f"UPDATE personal_language SET language = '{language}' WHERE user_id = {ctx.author.id}"
        )
      else:
      ...
      EmbedMessage = Embed(
        description="Your personal language has deleted.\nIt will now use the discord client language.",
        color=0x5865F2,
      )
      await cursor.execute(
        f"DELETE FROM personal_language WHERE user_id = {ctx.author.id}"
      )
