...
class ServerStats(Cog):
  ...
  async def serverstats(self, ctx: ApplicationContext):
    ...
    await self.bot.cursor.execute(
      f"SELECT * FROM default_guild_language WHERE guild_id = {ctx.guild.id}"
    )
    default_guild_language = await self.bot.cursor.fetchall()

    await self.bot.cursor.execute(
      f"SELECT * FROM reversed WHERE guild_id = {ctx.guild.id}"
    )
    reversed = await self.bot.cursor.fetchall()

    await self.bot.cursor.execute(
      f"SELECT * FROM force_reaction WHERE guild_id = {ctx.guild.id}"
    )
    force_reaction = await self.bot.cursor.fetchall()

    await self.bot.cursor.execute(
      f"SELECT * FROM channel_language WHERE guild_id = {ctx.guild.id}"
    )
    ...
    await self.bot.cursor.execute(
      f"SELECT * FROM langinfo WHERE guild_id = {ctx.guild.id}"
    )
    ...
    if langinfo != []:
      EmbedMessage.add_field(
        name="Language Infos",
        value="Language informations are enabled.",
        inline=False,
      )
