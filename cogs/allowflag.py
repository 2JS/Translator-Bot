...
class ReactionFlag(Cog):
  ...
  async def allowflag(
    ...
    EmbedDone = Embed(description="Reaction flag setting has been set", color=0x00FF00)
    ...
    if allowed is not None:
      await self.bot.cursor.execute(
        f"SELECT reaction_activated FROM default_guild_language WHERE guild_id = {ctx.guild.id}"
      )
    ...
    if force is not None:
      await self.bot.cursor.execute(
        f"SELECT forced FROM force_reaction WHERE guild_id = {ctx.guild.id}"
      )
    ...
    if minimalist is not None:
      await self.bot.cursor.execute(
        f"SELECT minimalist FROM force_reaction WHERE guild_id = {ctx.guild.id}"
      )
      minimalisted = await self.bot.cursor.fetchone()

      if minimalist == "no":
        sql = (
          f"UPDATE force_reaction SET minimalist = NULL WHERE guild_id = {ctx.guild.id}"
        )
        await self.bot.cursor.execute(sql)
        await self.bot.db.commit()
    ...
    if timeout is not None:
      await self.bot.cursor.execute(
        f"SELECT timeout FROM force_reaction WHERE guild_id = {ctx.guild.id}"
      )
      ...
      if timeout > 0:
        ...
        sql = (
          f"UPDATE force_reaction SET timeout = NULL WHERE guild_id = {ctx.guild.id}"
        )
        await self.bot.cursor.execute(sql)
        await self.bot.db.commit()
