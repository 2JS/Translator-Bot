...
class Translate(Cog):
  ...
  async def get_languages(ctx: AutocompleteContext):
    filtered_languages = [
      lang for lang in LANGNAMES if lang.startswith(ctx.value.lower())
    ]
  ...
  def automod_rules_check(self, automod_rules, Traduction, author, channel):
    ...
    for rule in automod_rules:
      if any(role.id in rule.exempt_role_ids for role in author.roles) or (
        channel.id in rule.exempt_channel_ids
      ):
        continue

      if (
        isinstance(rule.trigger_metadata.keyword_filter, list)
        and (
          any(AutoModActionType.block_message == action.type for action in rule.actions)
        )
        and (not ban_word)
      ):
      ...
      if (isinstance(rule.trigger_metadata.regex_patterns, list)) and (not ban_word):
        for keyword in rule.trigger_metadata.regex_patterns:
          pattern = flpc.compile(keyword)
          if flpc.fmatch(pattern, Traduction.text):
            ban_word = True

      if (isinstance(rule.trigger_metadata.allow_list, list)) and (ban_word):
  ...
  async def translate(
    ...
    try:
      Traduction = await self.bot.trad.translate(
        text, dest=to_language, src=from_language
      )
      ...
      try:
        ...
        await ctx.respond(
          "An error occured while translating the text.\nPlease try again.",
          ephemeral=ephemeral,
          delete_after=3 if not ephemeral else None,
        )
        return
    ...
    if ctx.interaction.context == InteractionContextType.guild:
      try:
        ...
        rule, ban_word = await self.bot.loop.run_in_executor(
          None,
          self.automod_rules_check,
          automod_rules,
          Traduction,
          ctx.author,
          ctx.channel,
        ...
        if ban_word:
          await ctx.respond(
            "An automod rule have been set up to ban a word you have in the translation.",
            ephemeral=ephemeral,
            delete_after=3 if not ephemeral else None,
          )
          ...
          for action in rule.actions:
            if AutoModActionType.send_alert_message == action.type:
              ...
              await alert_channel.send(
                embeds=[EmbedAlert, EmbedOriginal, EmbedTraduction]
              )
          return
      except Exception:
        pass
    ...
    if len(Traduction.text) > 2000:
      ...
      await ctx.respond(
        "The translated text is empty.\nI can not translate this.\nIf you think this is not the normal comportment, please open a ticket in the support server",
        ephemeral=ephemeral,
      )
      ...
      try:
        ...
        if "Message was blocked by AutoMod" in str(e):
          await ctx.respond(
            "The content of the translated has been blocked by Automod.",
            ephemeral=ephemeral,
            delete_after=3 if not ephemeral else None,
          )
        else:
