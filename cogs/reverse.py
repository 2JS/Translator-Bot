...
class ReverseLanguages(Cog):
  ...
  async def reverse_add(self, ctx: ApplicationContext, channel, language_1, language_2):
    ...
    if language_1 == language_2:
      ...
      await self.bot.cursor.execute(
        f"SELECT * FROM reversed WHERE guild_id = {ctx.guild.id} AND channel_id = {channel.id}"
      )
  ...
  #################
  @reverse.command(
    name="list", description="List all the reversed options in the server."
  )
  async def reverse_list(self, ctx: ApplicationContext):
    ...
    if ReversedLanguagesList == []:
      ...
      EmbedMessage = Embed(title="Reversed translation list", color=0x5865F2)
