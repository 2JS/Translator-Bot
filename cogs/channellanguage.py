...
class ChannelLanguage(Cog):
  ...
  async def get_languages(ctx: AutocompleteContext):
    filtered_languages = [
      lang for lang in LANGUAGES if lang.startswith(ctx.value.lower())
    ]
  ...
  @channellanguage.command(
    name="list", description="Give the different channel languages you set up"
  )
  async def channellanguage_list(self, ctx: ApplicationContext):
    ...
    await self.bot.cursor.execute(
      f"SELECT channel_id, language FROM channel_language WHERE guild_id = {ctx.guild.id}"
    )
    ...
    if ChannelLanguageList == []:
      ...
      if (
        len(ChannelLanguageList) <= 25
      ):  # Si il y a moins de 25 ou 25 channels configurés
        for channel in ChannelLanguageList:
          EmbedMessage.add_field(
            name=f"<#{channel[0]}>", value=f"{channel[1]}", inline=True
          )
        ...
        for x in range(24, len(ChannelLanguageList)):
          EmbedMessage.description += (
            f"<#{ChannelLanguageList[x][0]}> : {ChannelLanguageList[x][1]}\n"
          )
  ...
  @channellanguage.command(
    name="add", description="Add a different channel languages to your server"
  )
  @option(
    name="channel",
  ...
  async def channellanguage_add(self, ctx: ApplicationContext, channel, language):
    ...
    if language not in LANGUAGES:
      ...
      await self.bot.cursor.execute(
        f"SELECT language FROM channel_language WHERE guild_id = {ctx.guild.id} AND channel_id = {channel.id}"
      )
      channelalreadyexist = await self.bot.cursor.fetchone()

      if str(channelalreadyexist) == "None" or str(channelalreadyexist) == "(None,)":
        sql = (
          "INSERT INTO channel_language(guild_id, channel_id, language) VALUES(?,?,?)"
        )
        val = (ctx.guild.id, channel.id, language)
        await self.bot.cursor.execute(sql, val)
        await self.bot.db.commit()

        EmbedMessage = Embed(description="Language has been set !", color=0x00FF00)
        ...
        EmbedMessage = Embed(description="Language has been updated !", color=0x00FF00)
  ...
  @channellanguage.command(
    name="remove", description="Remove a different channel languages you set up"
  )
  @option(
    name="channel",
  ...
  async def channellanguage_remove(self, ctx: ApplicationContext, channel):
    ...
    if str(ischannelinlist) == "None" or str(ischannelinlist) == "(None,)":
      ...
      await self.bot.cursor.execute(
        f"DELETE FROM channel_language WHERE guild_id = {ctx.guild.id} AND channel_id = {channel.id}"
      )
      await self.bot.db.commit()
