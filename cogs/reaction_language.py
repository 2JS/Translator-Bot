...
class ReactionEvent(Cog):
  ...
  async def on_raw_reaction_add(self, payload):
    ...
    await cursor.execute(
      f"SELECT reaction_activated FROM default_guild_language WHERE guild_id = {payload.guild_id}"
    )
    reaction_allowed = await cursor.fetchone()

    if (
      str(reaction_allowed) == "None"
      or str(reaction_allowed) == "(None,)"
      or str(reaction_allowed[0]) == "enabled"
    ):
      pass
    else:
      await cursor.close()
    ...
    await cursor.execute(
      f"SELECT forced FROM force_reaction WHERE guild_id = {payload.guild_id}"
    )
    ...
    await cursor.execute(
      f"SELECT minimalist FROM force_reaction WHERE guild_id = {payload.guild_id}"
    )
    ...
    await cursor.execute(
      f"SELECT timeout FROM force_reaction WHERE guild_id = {payload.guild_id}"
    )
    ...
    if payload.emoji.name in self.bot.FLAG_CODES.keys():
      ...
      if TranslateMessage is None:
        try:
          TranslateMessage = await self.bot.get_channel(
            payload.channel_id
      ...
      try:
        Traduction = await self.bot.trad.translate(
          text=TranslateMessage.content, dest=source
        )
      except RateLimitError:
        raise
      except Exception:
        try:
          Traduction = await self.bot.trad.translate_to_detect(
            text=TranslateMessage.content, dest=source
          )
        except RateLimitError:
          raise
      ...
      EmbedImpossibleSendDM.set_author(
        ...
        else "https://cdn.pfps.gg/pfps/4909-default-discord.png",
      )

      if DMOnOff is None:
        await cursor.execute(
          f"SELECT yesno FROM DMUser WHERE user_id = {payload.user_id}"
        )
      ...
      if Traduction.src == source:
        ...
        EmbedSameLanguage.set_author(
          ...
          icon_url=MessageAuthor.avatar.url
          if MessageAuthor.avatar is not None
          else "https://cdn.pfps.gg/pfps/4909-default-discord.png",
        )

        try:
          await TranslateMessage.remove_reaction(
            emoji=f"{payload.emoji.name}", member=payload.member
          )
        except Exception:
          pass
        ...
        await cursor.execute(
          f"SELECT info FROM langinfo WHERE guild_id = {payload.guild_id}"
        )
        langinfo = await cursor.fetchone()
        if str(langinfo) != "None":
          langinfo = langinfo[0]

        if minimalist == "yes" and (
          str(DMOnOff) == "channel" or str(DMOnOff) == "('1',)"
        ):
          try:
            await TranslateMessage.remove_reaction(
              emoji=f"{payload.emoji.name}", member=payload.member
          ...
          if langinfo == "enabled":
            flag = (
              self.bot.LANGUAGES[Traduction.src]
              if Traduction.src != "auto"
              else "not found"
            )
          ...
          if len(Traduction.text) > 2000:
            ...
            for x in range(len(parts)):
              if x == 0:
                ...
                await self.bot.get_channel(payload.channel_id).send(
                  content=parts[x], delete_after=timeout
                )
          else:
            try:
              await TranslateMessage.reply(
        ...
        EmbedTranslated = Embed(title="The translation you requested", color=0x5865F2)
        ...
        content = await self.create_embed_with_fields(
          TranslateMessage.content, "**Original Message :**"
        )
        Embeds.append(content)

        content = await self.create_embed_with_fields(
          Traduction.text, "**Translated Message :**"
        )
        Embeds.append(content)
        ...
        if (str(DMOnOff) == "None") or (str(DMOnOff) == "(None,)"):
          try:
            ...
            try:
              await self.bot.get_channel(payload.channel_id).send(
                embed=EmbedImpossibleSendDM, delete_after=timeout
              )
              await cursor.close()
              return
            except Exception:
          ...
          try:
            if (len(Traduction.text) + len(TranslateMessage.content)) > 5500:
              await self.bot.get_channel(payload.channel_id).send(
                embeds=[Embeds[0], Embeds[1]], delete_after=timeout
              )
              await self.bot.get_channel(payload.channel_id).send(
                embeds=[Embeds[2], Embeds[3]], delete_after=timeout
              )
              await cursor.close()
              return
            else:
              await self.bot.get_channel(payload.channel_id).send(
                embeds=Embeds, delete_after=timeout
              )
              await cursor.close()
              return
          except Exception:
