...
class MessageTranslator(Cog):
  ...
  async def returnmessage(self, texte):
    ...
    if (
      "\n" in MessageEmojis
    ):  # Si le message contient des retours à la ligne, on les supprime
    ...
    for emoji_original in emojis_original:
      ...
      for emoji_translated in emojis_translated:
        if emoji_id in emoji_translated:
          translated_message = translated_message.replace(
            emoji_translated, emoji_original
          )
  ...
  async def on_message(self, message):
    ...
    if (
      type(message.channel) is Thread
      ...
      if (langue_originale == SOURCE_LANG) or (
        ...
        try:  # On tente de traduire le message
          Traduction = await self.bot.trad.translate(
            text=message.content, dest=DESTINATION_LANG
          )
        except RateLimitError:
          raise
        except Exception:
        ...
        is_same_message_content = await self.bot.loop.run_in_executor(
          None, self.compare_messages, message.content, Traduction.text
        )
        is_same_message_only = await self.bot.loop.run_in_executor(
          None, self.compare_messages, MessageOnly, Traduction.text
        )
        if is_same_message_content or is_same_message_only:
          Traduction.text = message.content

        if (langinfo == "enabled") and (
          True not in [is_same_message_content, is_same_message_only]
        ):
        ...
        async with aiohttp.ClientSession() as session:
          try:
            ...
            try:
              ...
              await cursor.execute(
                f"DELETE FROM linkchannels WHERE guild_id = {message.guild.id} AND channel_id_1 = {DESTINATION_CHANNEL}"
              )
              await self.bot.db.commit()
              await cursor.close()
              return
    ...
    await cursor.execute(
      f"SELECT language_1, language_2 FROM reversed WHERE guild_id = {message.guild.id} AND channel_id = {channel_id}"
    )
    result = await cursor.fetchone()
    if result is not None:
      result = list(result)
    ...
    if DESTINATION is None:
      ...
      await cursor.execute(
        f"SELECT language FROM channel_language WHERE guild_id = {message.guild.id} AND channel_id = {channel_id}"
      )
      result = await cursor.fetchone()
    ...
    if DESTINATION is None:
      ...
      await cursor.execute(
        f"SELECT default_language FROM default_guild_language WHERE guild_id = {message.guild.id}"
      )
      result = await cursor.fetchone()
    ...
    if DESTINATION is not None:
      ...
      try:
        Traduction = await self.bot.trad.translate(
          text=message.content, dest=DESTINATION
        )
      except RateLimitError:
        raise
      except Exception:
      ...
      if (
        Traduction.src == DESTINATION
      ):  # Si la langue source est la même que la langue de traduction, on ne fait rien
        await cursor.close()
        return
      elif (
        message.content == Traduction.text
      ):  # Si le contenu du message est le même que la traduction, on ne fait rien
        await cursor.close()
        return
        ...
        is_same_message_content or is_same_message_only
      ):  # Si le message traduit a une séquence trop ressemblante au message original, on ne fait rien
        await cursor.close()
        return
      else:
        Traduction.text = await self.is_custom_emoji(message.content, Traduction.text)
