...
class DefaultLanguage(Cog):
  ...
  async def defaultlanguage_set(self, ctx: ApplicationContext, language):
    ...
    await self.bot.cursor.execute(
      f"SELECT default_language FROM default_guild_language WHERE guild_id = {ctx.guild.id}"
    )
    ...
    if language in LANGUAGES:
      if str(DefaultLG) == "None":
        sql = (
          "INSERT INTO default_guild_language(guild_id, default_language) VALUES(?,?)"
        )
        val = (ctx.guild.id, language)
        await self.bot.cursor.execute(sql, val)
        await self.bot.db.commit()

        EmbedLanguage = Embed(description="Default Language set !", color=0x00FF00)

      else:
        sql = (
          "UPDATE default_guild_language SET default_language = ? WHERE guild_id = ?"
        )
        val = (language, ctx.guild.id)
        await self.bot.cursor.execute(sql, val)
        await self.bot.db.commit()

        EmbedLanguage = Embed(description="Default Language updated !", color=0x00FF00)
  ...
  @defaultlanguage.command(
    name="reset", description="Used to remove the default language of the server"
  )
  async def defaultlanguage_reset(self, ctx: ApplicationContext):
    ...
    if str(DefaultLG) == "None" or str(DefaultLG) == "(None,)":
      EmbedDeleted = Embed(
        description="You didn't put any default language before", color=0xFF0000
      )
      ...
      EmbedDeleted = Embed(
        description="Default Language has been deleted !", color=0x00FF00
      )
  ...
  @defaultlanguage.command(
    name="see", description="Used to see the default language of the server"
  )
